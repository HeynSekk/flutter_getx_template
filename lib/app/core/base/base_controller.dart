...
abstract class BaseController extends GetxController {
  ...
  dynamic callDataService<T>(
    ...
    try {
      ...
      exceptionVar = exception;
      if (exception.message != "") showErrorMessage(exception.message);
    } on AppException catch (exception) {
      exceptionVar = exception;
      showErrorMessage(exception.message);
    } catch (error) {
      exceptionVar = AppException(message: "$error");
      logger.e("Controller>>>>>> error $error");
    }

    //if exceptionVar is not null, then may log the error

    if (onError != null) onError(exceptionVar);
